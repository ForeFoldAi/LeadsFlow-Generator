...
# ── Sector detection (for Additional Notes only — no Sector column in template) ─
...
def lead_to_leadsflow(lead: Lead, keyword: str = "", country: str = "India") -> Optional[dict]:
    ...
    # Customer Interested In: use website domain or category
    website_raw   = (lead.website or "").strip()
    domain_match  = re.search(r"(?:https?://)?(?:www\.)?([^/\s?#]+)", website_raw)
    interested_in = _sanitize(
    ...
    return {
        ...
        "Last Contacted By":                                                                 "",
        "Next Followup Date (YYYY-MM-DD)":                                                  "",
        "Customer Interested In":                                                            interested_in,
